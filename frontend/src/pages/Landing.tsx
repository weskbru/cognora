...
import {
  ...
  Check, Star, ChevronDown, BookX, BarChart3, Target,
} from 'lucide-react';
...
const FEATURES = [
  {
    ...
    bg: 'bg-indigo-50',
    title: 'IA Generativa',
    desc: 'Envie qualquer PDF ou texto e a IA gera questões, resumos e flashcards personalizados para você.',
  },
  {
    icon: HelpCircle,
    ...
    desc: 'Múltipla escolha ou verdadeiro/falso gerados automaticamente a partir do seu conteúdo. Pratique onde quiser.',
  },
  {
    icon: BookX,
    color: 'text-rose-500',
    ...
    desc: 'Todo erro vira oportunidade. O sistema registra automaticamente o que você errou para revisar depois.',
  },
  {
    icon: Swords,
  ...
  return (
    <Section id="features" className="py-28 px-6 max-w-6xl mx-auto">
      <motion.div variants={fadeUp} className="text-center mb-16">
        <Badge variant="outline" className="mb-4 bg-violet-50 text-violet-600 border-violet-200 text-xs px-3 py-1 rounded-full">
          Recursos
        </Badge>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {FEATURES.map((f) => (
          <motion.div
            key={f.title}
            variants={fadeUp}
            className="group p-6 rounded-2xl border border-slate-100 bg-white hover:shadow-md hover:-translate-y-0.5 transition-all duration-300"
          >
...
const PREVIEW_PLANS = [
  {
    ...
    price: 'R$0',
    period: 'para sempre',
    features: ['3 gerações por dia', 'Até 2 matérias', 'Questões ilimitadas', 'Competições'],
    cta: 'Começar grátis',
    highlight: false,
  },
  {
    ...
    period: '/mês',
    badge: 'Mais popular',
    features: ['20 gerações por dia', 'Matérias ilimitadas', 'Documentos ilimitados', 'Suporte prioritário'],
    cta: 'Assinar Pro',
    highlight: true,
  },
  {
    ...
    price: 'R$39,90',
    period: '/mês',
    features: ['Gerações ilimitadas', 'Tudo do Pro', 'Acesso antecipado', 'Sem restrições'],
