...
export default function CompetitionResults({
  ...
  userEmail,
  live = false,
}: CompetitionResultsProps): ReactElement | null {
        ...
        {sorted.map((p, i) => (
          <div key={p.email} className={`flex items-center gap-3 p-3 rounded-xl transition-all ${
            p.email === userEmail
              ...
              : i === 0 && !live
              ? 'bg-amber-50 border border-amber-200/60 dark:bg-amber-950/30 dark:border-amber-800/50'
              : 'bg-secondary/50'
          }`}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-0.5 text-xs text-muted-foreground">
                <span className="flex items-center gap-0.5 text-emerald-600 dark:text-emerald-400"><Target className="h-3 w-3" />{p.correct} acertos</span>
            ...
            <div className="text-right shrink-0">
              <p className="font-bold text-amber-600 text-base dark:text-amber-400">{p.score}</p>
